...
class CNC:
    ...
    def loadConfig(config):
        ...
        try:
            ...
            pass
        try:
            CNC.doublesizeicon = bool(int(config.get(section, "doublesizeicon")))
        except:
            pass
        try:
...
class GCode:
    ...
    def saveSVG(self, filename):
        ...
        svg.write('<!-- SVG generated by bCNC: https://github.com/vlachoudis/bCNC -->\n')
        svg.write('<svg viewBox="%s %s %s %s">\n' % (
            (minx * scale) - padding, (-maxy * scale) - padding, ((maxx - minx) * scale) + padding * 2,
            ...
            for line in block:
                ...
                if self.cnc.gcode == 1:  # line
                    ...
                    if self.cnc.y == self.cnc.yval and self.cnc.x == self.cnc.xval:  # is full circle?
                        ...
                        svgpath += svgArc(scale, self.cnc.gcode, self.cnc.rval, self.cnc.x, -self.cnc.y, midx, -midy,
                                          xc, -yc)
                    # Finish arc
                    svgpath += svgArc(scale, self.cnc.gcode, self.cnc.rval, midx, -midy, self.cnc.xval, -self.cnc.yval,
                                      xc, -yc)
                self.cnc.motionEnd()
    ...
    # ----------------------------------------------------------------------
    def fromPath(self, path, block=None, z=None, retract=True, entry=False, exit=True, zstart=None, ramp=None,
                 comments=True, exitpoint=None, truncate=None):
        # Recursion for multiple paths
        if not isinstance(path, Path):
            block = Block("new")
            for p in path:
                block.extend(
                    self.fromPath(p, None, z, retract, entry, exit, zstart, ramp, comments, exitpoint, truncate))
        ...
        def addSegment(segment, z=None, cm=""):
            ...
            if segment.type == Segment.LINE:
                ...
                if z is None:
                    block.append("g1 %s %s" % (self.fmt("x", x, 7), self.fmt("y", y, 7)) + cm)
                else:
                    block.append("g1 %s %s %s" % (self.fmt("x", x, 7), self.fmt("y", y, 7), self.fmt("z", z, 7)) + cm)
            ...
            for sid, segment in enumerate(path):
                ...
                if ztab != ztabprev:  # has tab height changed? tab boundary crossed?
                    if (ztab == float("-inf") or ztab < ztabprev) and (
                            zh < ztabprev or zhprev < ztabprev):  # if we need to enter the toolpath after done clearing the tab
    ...
    def cutPath(self, newblock, block, path, z, depth, stepz, helix=False, helixBottom=True, ramp=0, islandPaths=[],
                exitpoint=None, springPass=False):
    ...
    # ----------------------------------------------------------------------
    def cut(self, items, depth=None, stepz=None, surface=None, feed=None, feedz=None, cutFromTop=False, helix=False,
            helixBottom=True, ramp=0, islandsLeave=False, islandsCut=False, islandsSelectedOnly=True, exitpoint=None,
            springPass=False, islandsCompensate=False):
            ...
            for path in self.toPath(bid):
                ...
                if cutFromTop:
                    self.cutPath(newblock, block, path, surface + stepz, depth, stepz, helix, helixBottom, ramp,
                                 islandPathsClean, exitpoint, springPass)
                else:
    ...
    # return new blocks inside the blocks list
    # ----------------------------------------------------------------------
    def trochprofile_cnc(self, blocks, offset, overcut=False, adaptative=True, adaptedRadius=0.0, cutDiam=0.0,
                         tooldiameter=0.0, \
    ...
    # ----------------------------------------------------------------------
    def adaptative_clearence(self, blocks, offset, overcut=False, adaptative=True, adaptedRadius=0.0, cutDiam=0.0,
                             tooldiameter=0.0, name=None):
        undoinfo = []
        msg = ""
        newblocks = []
        for bid in reversed(blocks):
